...
<template>
  <li class="flex justify-between items-center border-b border-gray-500 pb-2">
    <div class="space-y-1">
      ...
      <p class="text-gray-600 font-semibold">Cliente: {{ payment.customer.name }} {{ payment.customer.lastName }}</p>
      <p v-if="payment.customer.reference" class="text-gray-600 font-semibold">Referencia: {{ payment.customer.reference }}</p>
      <button
        type="submit"
        class="text-white font-semibold bg-green-400 py-1 px-7 rounded-md text-center"
        :class="payment.state ? 'bg-green-400' : 'bg-red-400'"
      >{{ payment.state ? 'Pagado' : 'Pendiente' }}
      </button>
    </div>
